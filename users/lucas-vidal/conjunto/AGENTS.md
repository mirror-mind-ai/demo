...
For non-trivial work, follow the full lifecycle as described in `docs/process/development-guide.md`. The phases below summarize what the Driver must produce at each stage.

## Story Lifecycle

### 1. Read and Orient

Read the project context files listed above. Identify the current state: what version is current, what work is next, what the roadmap says. Run the opening ritual from the development guide.
...
Write the `plan.md` for the story.

**→ Checkpoint 1: stop and present the plan. Wait for Navigator confirmation before writing any code.**

### 3. Implement

Write code following the plan. Keep scope stable. If new work surfaces, distinguish what blocks the current story from what should become follow-up work.

### 4. Test and Validate

Run `npm test` and `npm run build`. For user-visible work, prepare a manual validation route: `npm run dev`, URLs to visit, expected observations, and what to check.

Present:

- **Files changed** — list of modified and new files.
- **Test results** — which tests ran, pass/fail count, `tsc --noEmit` result.
...
Review what was built. Write the `refactoring.md` for the story. Assess:
...
Update all pending documentation. Write the `test-guide.md` for the story. Update `index.md` for the story, epic index, roadmap index, `docs/index.md`, `README.md`, `worklog.md`, release note, and `CURRENT_VERSION`.

Then run the coherence check — ask what was forgotten:
...
- Does the version citation sweep pass? (Every `vX.Y.Z` mention links to its release note.)
