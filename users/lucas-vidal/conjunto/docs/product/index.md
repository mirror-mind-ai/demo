# Produto

O produto Conjunto é o veículo pelo qual a comunidade existe na
prática. Sem ele, a ideia de uma comunidade fechada e curada para
líderes técnicos seria só uma boa intenção. Com ele, encontros
mensais acontecem, fios persistem, biblioteca cresce, e a relação
entre membros tem um lugar onde aparecer.

## Para entender melhor

- **[O problema](problem.md)** — a solidão específica do líder
  técnico e por que os interlocutores naturais quase nunca servem
  para o tipo de conversa que importa.
- **[A solução](solution.md)** — como o desenho responde ao
  problema e o que o produto entrega hoje em v0.1.0.
- **[Princípios](principles.md)** — as decisões de design que mantêm
  o produto coerente com sua intenção, em vez de derivar para "mais
  uma plataforma de comunidade".
